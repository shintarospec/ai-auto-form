...
- **Productモデル拡張**
  - 送信者情報カラム追加: `sender_name`, `sender_email`, `sender_company`, `sender_phone`
  - 案件ごとに異なる送信者情報を設定可能に（従来は固定値）

- **案件登録・編集機能**
  - `POST /api/simple/products` エンドポイント新規作成
  - 送信者情報の必須バリデーション実装
  - 案件名の重複チェック
  - `PUT /api/simple/products/<id>` に送信者情報更新処理追加
...
- **タスク生成ロジック改善**
  - タスク生成時に `product.sender_*` フィールドから送信者情報を取得
  - form_dataに案件固有の送信者情報を設定

#### Fixed（修正）

- フォーム自動入力でname/email/company/phoneが空になる問題を修正
  - 原因: form_dataキー名不一致（`sender_company` → `company`）
  - 解決: キー名を統一し、Productモデルに送信者情報を追加

#### Database（データベース）

- **マイグレーション実行**: 2025-12-31
  - simple_productsテーブルに4カラム追加
  - 既存4商品にデフォルト送信者情報を設定（山田太郎, yamada@example.com, 株式会社テスト, 03-1234-5678）
